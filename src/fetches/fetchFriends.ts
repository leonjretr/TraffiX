import {IFriendsApiResponse} from "../components/types/types.tsx";
import {DOMAIN_ADDRESS} from "../config/constants.ts";

export default async function fetchFriends(): Promise<IFriendsApiResponse | undefined> {
    ...
    try {
        const response = await fetch(`${DOMAIN_ADDRESS}api/v1/users/me/friends/`, {
            headers: {
